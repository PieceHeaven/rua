<template>
  <div class="wrap">
    ...
    <van-row style="background: white; border-radius: 10px">
      ...
      <van-col span="12">
        <div class="headerInfo">
          ...
          <div><van-icon name="location-o" />{{ selectLeader.address }}</div>
        </div>
      </van-col>
      <van-col span="6">
        <div class="iconWrap">
      ...
      <van-grid :border="false" :column-num="5">
        <van-grid-item v-for="(item, index) in categoryList" :key="index">
          <van-image
            width="50px"
            height="50px"
            :src="host + '/category/' + item.categoryIcon"
            round
          />
          {{ item.categoryName }}
        </van-grid-item>
    ...
    <div class="zc">
      <van-row>
        <van-col span="12">
          <div class="firstZcTitle" style="text-align: left; text-indent: 23px">
            立顿专场
          </div>
          <div class="firstZcText" style="text-align: left; text-indent: 23px">
            立顿专场
          </div>
          <div>
            <img
    ...
    <div class="foodsItem">
      ...
      >
        <van-row>
          <van-col span="10">
            <img :src="host + '/foodsDetail/' + item.foodsBanner[0]" alt />
          </van-col>
          <van-col span="14">
            <div class="foodsTitle">
              {{ item.foodsName }}&nbsp;&nbsp;&nbsp;{{ item.foodsSpecifications }}
            </div>
            <div class="smlText">{{ item.foodsText }}</div>
            <div class="sendTime">
              <div>送货时间</div>
              <div>
                {{ item.foodsDelivery.split("-")[1] }}月{{
                  item.foodsDelivery.split("-")[2]
                }}号
              </div>
            </div>
            <div>
  ...
  mounted() {
    ...
    if (location) {
      ...
      if (selectLeader) {
        ...
        this.leaderHeader = this.host + "/userHead/" + selectLeader.userId.userHeader;
      } else {
        this.$router.push({
          path: "/selectLeader",
        });
