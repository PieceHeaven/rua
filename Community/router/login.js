module.exports = (app) => {
    ...
    login.post('/register', async (ctx) => {
        //后台管理系统注册
        const { code, name, userHeader, money, mail, password } = ctx.request.body
        ...
        if (data) {
            if (data == code) {
                ...
                await user.create({
                    name, userHeader, money, mail, password: signPassWord
                });
                ctx.body = {
                    code: 0,
                    msg: '创建成功'
    ...
    login.post('/upload', async ctx => {
        // console.log("upload ctx...");
        // console.log(ctx.request.body);
        // const image = new Image({ image: ctx.request.body.image });
        // try {
        //     await image.save();
        //     ctx.status = 201;
        //     ctx.body = { message: 'Image saved successfully' };
        // } catch (error) {
        //     ctx.status = 400;
        //     ctx.body = { error };
        // }
