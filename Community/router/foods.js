module.exports = (app) => {
    ...
    let hasFolied = fs.existsSync("./uploads/foodsDetail")
    hasFolied ? null : fs.mkdir('./uploads/foodsDetail', () => { })

    let storage = multer.diskStorage({
        //文件保存路径
        destination: function (req, file, cb) {
            cb(null, 'uploads/foodsDetail')
        },
        //修改文件名称
    ...
    Foods.post('/delete', async ctx => {
        ...
        if (arr.length > 0) {
            for (let i = 0; i < arr.length; i++) {
                fs.unlink(`./uploads/foodsDetail/${banner[i]}`, (err) => {
